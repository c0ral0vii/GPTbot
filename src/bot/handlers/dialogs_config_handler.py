...
async def select_dialog_handler(callback: types.CallbackQuery, state: FSMContext):
    ...
    if dialog:
        await callback.message.answer(
            ...
            f"Выберите действие:",
            reply_markup=await dialog_config_keyboard(dialog_id=dialog.id, dialog_uuid=dialog.uuid),
        )
    else:
...
async def change_title_handler(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        "Введите новое название для диалога(до 128 символов):"
    )
...
async def delete_dialog(callback: types.CallbackQuery, state: FSMContext):
    ...
    if success:
        ...
        await callback.message.answer(
            "Ошибка: Диалог не найден или не может быть удален."
        )

    await callback.message.delete()


@router.callback_query(F.data.startswith("get_full_dialog_"))
async def get_full_dialog_handler(callback: types.CallbackQuery, state: FSMContext):
    """Получение ссылки на весь диалог для пользователя"""
    dialog_id = callback.data.split("_")[-1]
    dialog = await DialogORM.get_dialog(dialog_id=int(dialog_id))
    
    if dialog:
        await callback.message.answer(f"Ссылка на диалог: {dialog.uuid}")
    else:
        await callback.message.answer("Диалог не найден")
